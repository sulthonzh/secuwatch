...
export class ProjectCommand {
  ...
  private async addProject(config: ConfigManager, projectName: string) {
    try {
  ...
  private async removeProject(config: ConfigManager, projectName: string) {
    try {
  ...
  private async listProjects(config: ConfigManager) {
    const projects = await config.getProjects();
  ...
  private async checkProjectStatus(config: ConfigManager, projectName: string) {
    try {
      const projects = await config.getProjects();
      const project = projects.find(p => p.name === projectName);
  ...
  private displayProjectStatus(project: any, result: ScanResult) {
    ...
    if (result.outdated && result.outdated.length > 0) {
      ...
      result.outdated.slice(0, 5).forEach((pkg: any) => {
  ...
  private async hasPackageJson(path: string): Promise<boolean> {
    try {
      const fs = await import('fs-extra');
  ...
  private async prompt(questions: any[]): Promise<any> {
